...
class mzitu():
	...
	def all_url(self,url):
		html = self.requests(url)#调用request函数把套图地址传进去会返回给我们一个response
	...
	def mkdir(self,path):
		path = path.strip()
		isExists = os.path.exists(os.path.join("C:\meitu",path))
		if not isExists:
			print(u'创建了一个名字叫做',path,u'的文件夹！')
			os.makedirs(os.path.join("C:\mzitu",path))
			return True
		else:
	...
	def request(self,url):
		content = request.get(url,headers=self.headers)
		return content
